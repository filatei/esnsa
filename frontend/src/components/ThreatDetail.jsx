    ...
    <div style={{ background:'var(--panel)', border:'1px solid var(--border-bright)', borderRadius:'8px', height:'100%', display:'flex', flexDirection:'column', overflow:'hidden' }}>
      <div style={{ background:'var(--header)', borderBottom:'1px solid var(--border)', padding:'12px 14px', display:'flex', justifyContent:'space-between', alignItems:'center' }}>
        <div style={{ display:'flex', gap:'8px', alignItems:'center' }}>
          <span style={{ color:'var(--accent)', fontFamily:'var(--font-mono)', fontSize:'13px', fontWeight:'700', letterSpacing:'1px' }}>{threat.incident_id}</span>
      ...
      <div style={{ padding:'14px', overflowY:'auto', flex:1 }}>
        <div style={{ fontFamily:'var(--font-display)', fontSize:'15px', fontWeight:'700', letterSpacing:'2px', color:'#ffffff', marginBottom:'12px', textTransform:'uppercase' }}>{threat.type}</div>
        ...
        {threat.agencies?.length > 0 && (
          <div style={{ marginTop:'12px' }}>
            <div style={{ fontSize:'10px', fontWeight:'700', fontFamily:'var(--font-mono)', color:'var(--text-dim)', letterSpacing:'2px', marginBottom:'6px', textTransform:'uppercase' }}>Assigned Agencies</div>
        ...
        {threat.description && (
          <div style={{ marginTop:'12px' }}>
            <div style={{ fontSize:'10px', fontWeight:'700', fontFamily:'var(--font-mono)', color:'var(--text-dim)', letterSpacing:'2px', marginBottom:'6px', textTransform:'uppercase' }}>Description</div>
        ...
        {threat.notes?.length > 0 && (
          <div style={{ marginTop:'12px' }}>
            <div style={{ fontSize:'10px', fontWeight:'700', fontFamily:'var(--font-mono)', color:'var(--text-dim)', letterSpacing:'2px', marginBottom:'6px', textTransform:'uppercase' }}>Timeline / Notes</div>
            {threat.notes.map(n => (
              <div key={n.id} style={{ padding:'8px', background:'var(--bg)', borderRadius:'4px', marginBottom:'6px' }}>
                <div style={{ fontSize:'10px', fontWeight:'600', fontFamily:'var(--font-mono)', color:'var(--text-dim)', marginBottom:'4px' }}>{n.author_name} · {new Date(n.created_at).toLocaleString()}</div>
                <div style={{ fontSize:'13px', fontWeight:'500', color:'#e8f5ee', lineHeight:1.6 }}>{n.note}</div>
              </div>
            ))}
          </div>
        )}
